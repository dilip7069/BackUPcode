...
app.service('ArchiveService', [ '$http', '$rootScope', function($http, $rootScope) {
	this.search = function(name, date) {
		$http.get("http://localhost:7080/springokay/archive/documents", {
			params : {
				person : name,
				date : date
			}
...
app.controller('UploadCtrl', [ '$scope', 'fileUpload',
		function($scope, fileUpload) {
			$scope.uploadFile = function() {
				...
				var uploadUrl = "http://localhost:7080/springokay/archive/upload";
...
app.run(function($rootScope, $http) {
	$http.get("http://localhost:7080/springokay/archive/documents").success(
			function(response) {
				$rootScope.metadataList = response;
			});
});
